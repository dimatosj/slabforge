...
- **`buildHumpMold(params): Mesh`** — a solid positive frustum of the **interior**: a closed N-gon frustum from radius `rb` at `y=0` to `rt` at `y=h`, with a bottom cap (fan, downward normals), a top cap (fan, upward normals), and `n` side quads (2 triangles each, outward normals). ~`4n` triangles. Flat base = the bottom cap.
- **`buildSlumpMold(params): Mesh`** — a prismatic block with a frustum cavity of the **exterior** (radii `rb+t`, `rt+t`): an outer N-gon prism (outer radius = `rt + t + margin`, `margin = t`) of total height `h + base` (`base = convertUnits(1, "cm", units)`), a flat bottom cap, a **top annular rim** connecting the outer top ring to the cavity mouth (radius `rt+t`), cavity side walls tapering down to the cavity floor (at `y = base`, radius `rb+t`), and a cavity floor cap. Inward-facing normals on the cavity surfaces, outward on the block exterior. Watertight.
